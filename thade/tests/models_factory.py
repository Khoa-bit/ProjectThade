...
class RecordFactory(factory.django.DjangoModelFactory):
    ...
    utc_trading_date = factory.Sequence(
        lambda n: timezone.now().replace(hour=2, minute=0, second=0, microsecond=0)
    ...
    close_vnd = factory.LazyAttribute(
        lambda this: this.reference_price_vnd + this.close_fluctuation
    )
    ...
    open_vnd = factory.LazyAttribute(
        lambda this: this.reference_price_vnd + this.open_fluctuation
    )

    highest_fluctuation = factory.Faker("random_int", min=0, max=2000, step=100)
    highest_vnd = factory.LazyAttribute(
        lambda this: max(this.close_vnd, this.open_vnd) + this.highest_fluctuation
    )

    lowest_fluctuation = factory.Faker("random_int", min=-2000, max=0, step=100)
    lowest_vnd = factory.LazyAttribute(
        lambda this: min(this.close_vnd, this.open_vnd) + this.lowest_fluctuation
    )
    ...
    if company is None:
        ...
        raise Exception(
            "Company already has records.\n{} has {} record(s)".format(
                company, company.record_set.count()
            )
        )
...
class BotFactory(factory.django.DjangoModelFactory):
    ...
    bid = factory.LazyAttribute(
        lambda this: "{}-{:%Y%m%d-%H%M%S%z}-{}".format(
            this.company.code, this.deploy_date, this.name.upper()
        )
    )
...
class BotLogFactory(factory.django.DjangoModelFactory):
    ...
    decimal_balance_vnd = factory.Faker(
        "pydecimal", min_value=0, max_value=50000000, right_digits=3
    )
    ...
    log_str = factory.LazyAttribute(
        lambda this: "{} - {}: {} __ {}\n".format(
            timezone.now(),
            this.last_updated_record.rid,
            this.signal,
            this.bot.company.code,
        )
    )
    decimal_investment_vnd = factory.Faker("random_int", min=0, max=50000000, step=1000)
    all_time_min_total_vnd = factory.Faker(
        "pydecimal", min_value=0, max_value=50000000, right_digits=3
    )
    all_time_max_total_vnd = factory.Faker(
        "pydecimal", min_value=0, max_value=50000000, right_digits=3
    )
    control_decimal_balance_vnd = factory.Faker(
        "pydecimal", min_value=0, max_value=50000000, right_digits=3
    )
