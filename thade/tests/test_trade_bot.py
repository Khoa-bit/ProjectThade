...
class AlgorithmTests(TestCase):
    ...
    def test_data_QuerySet(self):
        ...
        self.assertQuerysetEqual(
            abstract_algorithm.data, company.record_set.all(), ordered=False
        )


class MovingAverageTests(TestCase):
    def setUp(self):
        from thade.tests.records_fixture import (
            close_records,
            moving_50,
            moving_200,
            signals,
    ...
    def test_update_extract_compute_action_on_first_200_records(self):
        ...
        self.assertQuerysetEqual(
            moving_average.data, self.company.record_set.order_by("-utc_trading_date")
        )
    ...
    def test_compute_action_on_all_500_records(self):
        ...
        last_updated_record = self.company.record_set.order_by(
            "-utc_trading_date"
        ).last()
        ...
        while last_updated_record != newest_records:
            ...
            last_updated_record = (
                self.company.record_set.filter(
                    utc_trading_date__gt=last_updated_record.utc_trading_date
                )
            ...
            try:
                moving_average.update_data(
                    self.company.record_set.filter(
                        utc_trading_date__lte=last_updated_record.utc_trading_date
                    )
                )
...
class TradeBotTests(TestCase):
    ...
    def test_generated_bid(self):
        ...
        self.assertEqual(
            bot.bid,
            "{}-{:%Y%m%d-%H%M%S%z}-JESTER".format(self.company.code, AWARE_DATETIME),
        )

    def test_last_updated_record_from_deploy_date_after_officially_on_stock_market(
        self,
    ):
    ...
    def test_last_updated_record_from_deploy_date_before_officially_on_stock_market(
        self,
    ):
        with warnings.catch_warnings(record=True) as w:
            ...
            self.assertEqual(
                ...
                " {1} < {2}\n=> last_updated_record = {2}".format(
                    self.company, before_official_deploy_date, bot.last_updated_record
                ),
            )
    ...
    def test_input_invalid_last_updated_record(self):
        ...
        last_update_record_different_company = RecordFactory(
    ...
    def test_delete_all_log(self):
        ...
        bot.delete_all_logs()
        self.assertQuerysetEqual(
            Bot.objects.all(), [], "Bot model is removed from database"
        )
        self.assertEqual(
            BotLog.objects.count(),
            0,
            "All of its BotLog models are delete together with the Bot models",
        )
    ...
    def test_invest(self):
        ...
        self.assertEqual(
            round(bot.control_decimal_balance_vnd, 1), Decimal(200 * 1000000)
        )
        ...
        self.assertEqual(
            round(bot.control_decimal_balance_vnd, 1), Decimal(201 * 1000000)
        )
        self.assertTrue(
            os.path.exists(
                BASE_DIR
                / f"thade/trade_bot/logs/Jester_{self.company.code}_Algorithm.txt"
            ),
        ...
        self.assertEqual(
            round(bot.control_decimal_balance_vnd, 1), Decimal(206 * 1000000)
        )
        ...
        self.assertEqual(
            round(bl[1].control_decimal_balance_vnd, 1), Decimal(206 * 1000000)
        )
    ...
    def test_withdraw(self):
        ...
        self.assertEqual(
            round(bot.control_decimal_balance_vnd, 1), Decimal(199 * 1000000)
        )
        self.assertTrue(
            os.path.exists(
                BASE_DIR
        ...
        self.assertEqual(
            round(bot.control_decimal_balance_vnd, 1), Decimal(194 * 1000000)
        )
        ...
        self.assertEqual(
            round(bl.control_decimal_balance_vnd, 1), Decimal(194 * 1000000)
        )
    ...
    def test_action_BUY(self):
        ...
        self.assertEqual(
            round(bot.decimal_balance_vnd, 1), Decimal(200 * 1000000 - 5469075.0)
        )
    ...
    def test_write_txt(self):
        ...
        with open(
            BASE_DIR / f"thade/trade_bot/logs/Jester_{self.company.code}_Algorithm.txt",
            "r",
        ) as f:
    ...
    def test_run_when_bot_active(self):
        ...
        for i in range(31):
            temp_log = bot_log[i]
            self.assertEqual(
                temp_log.last_updated_record.utc_trading_date,
                AWARE_DATETIME.replace(hour=2, minute=0, second=0, microsecond=0)
                - timezone.timedelta(days=i),
            )
    ...
    def test_run_moving_average(self):
        ...
        test_bot_log_signals = BotLog.objects.order_by(
            "-last_updated_record__utc_trading_date"
        ).values_list("signal", flat=True)
        test_stocks = BotLog.objects.order_by("-last_updated_record__utc_trading_date")[
            :302
        ].values_list("stocks", flat=True)
        test_decimal_balance_vnd = BotLog.objects.order_by(
            "-last_updated_record__utc_trading_date"
        )[:302].values_list("decimal_balance_vnd", flat=True)
    ...
    def test_get_trade_bot(self):
        ...
        self.assertEqual(
            trade_bot.decimal_investment_vnd, last_log.decimal_investment_vnd
        )
        self.assertEqual(
            trade_bot.all_time_min_total_vnd, last_log.all_time_min_total_vnd
        )
        self.assertEqual(
            trade_bot.all_time_max_total_vnd, last_log.all_time_max_total_vnd
        )
        self.assertEqual(
            trade_bot.control_decimal_balance_vnd, last_log.control_decimal_balance_vnd
        )
